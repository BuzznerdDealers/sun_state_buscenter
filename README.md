...
The BuzzNerd dealer brand site for **Sun State Bus Centers** — the exclusive IC Bus
sales, parts and service dealer for the state of Florida. Generated from the BuzzNerd
dealer site template, so it carries **the renderer** — the single implementation that
turns site JSON into HTML — alongside this dealer's own content under `site/` and
`public/`.

Two pages are built here: the homepage (`site/pages/home`) and Financing & Warranty
(`site/pages/financing`). Chrome, the utility bar, the footer and the testimonial
rotator are in `site/templates/` and `site/sections/`; the location finder, the hero
action tiles and the social row are coded widgets in `site/widgets/`.
